...
// 生成球体的经纬线顶点数据和纹理坐标
void createSphereVertices(float* vertices, float* texCoords, int numSegments, int numLatitudes, int numLongitudes, float radius) {
    int vertexCount = 0;
    ...
    for (int i = 0; i < numLatitudes; ++i) {
        ...
        for (int j = 0; j < numSegments; ++j) {
            ...
            texCoords[vertexCount * 2] = static_cast<float>(j) / numSegments;
            texCoords[vertexCount * 2 + 1] = 1.0f - static_cast<float>(i + 1) / (numLatitudes + 1);
            vertexCount++;
        }
    }
    ...
    for (int i = 0; i < numLongitudes; ++i) {
        ...
        for (int j = 0; j < numSegments; ++j) {
            ...
            texCoords[vertexCount * 2] = static_cast<float>(i) / numLongitudes;
            texCoords[vertexCount * 2 + 1] = 1.0f - static_cast<float>(j) / (numSegments - 1);
            vertexCount++;
        }
    }
}

void createVBOVAO(unsigned int &VAO, unsigned int &VBO, unsigned int &texVBO, int numSegments, int numLatitudes, int numLongitudes, float radius) {
    int totalVertices = (numSegments * numLatitudes) + (numSegments * numLongitudes);
    float vertices[totalVertices * 3];
    float texCoords[totalVertices * 2];
    
    createSphereVertices(vertices, texCoords, numSegments, numLatitudes, numLongitudes, radius);
...
int main(int argc, const char * argv[]) {
    ...
    // 球体参数
    int numSegments = 50 * 2;    // 经度数
    int numLatitudes = 50;   // 纬度数
    int numLongitudes = 50;  // 使用的经线数
    float radius = 0.5f;     // 半径
    
    // 生成和配置 VAO 和 VBO
    unsigned int VAO, VBO, texVBO;
    createVBOVAO(VAO, VBO, texVBO, numSegments, numLatitudes, numLongitudes, radius);
